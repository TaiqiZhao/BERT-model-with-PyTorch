...
def model_eval_sts(dataloader, model, device):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc=f'eval sts', disable=TQDM_DISABLE)):
        (b_ids1, b_mask1,
            b_ids2, b_mask2,
...
# Perform model evaluation on different tasks seperately, can be processed on train set and dev set.
def model_eval_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'eval para', disable=TQDM_DISABLE)):
            ...
            y_hat = logits.round().flatten().cpu().numpy()
            b_labels = b_labels.flatten().cpu().numpy()
...
# Perform model evaluation on different tasks separately, can be only processed on test set.
